...
func ReplacenamespacedcustomobjectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplacenamespacedcustomobjectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_group_version_namespaces_namespace_plural_name",
		mcp.WithDescription("replace the specified namespace scoped custom object"),
		mcp.WithObject("body", mcp.Required(), mcp.Description("The JSON schema of the Resource to replace.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReplacenamespacedcustomobjectHandler(cfg),
	}
}
