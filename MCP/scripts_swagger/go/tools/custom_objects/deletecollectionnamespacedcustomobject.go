...
func DeletecollectionnamespacedcustomobjectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/%s/%s/namespaces/%s/%s%s", cfg.BaseURL, queryString)
...
func CreateDeletecollectionnamespacedcustomobjectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_group_version_namespaces_namespace_plural",
		mcp.WithDescription("Delete collection of namespace scoped custom objects"),
		...
		mcp.WithString("propagationPolicy", mcp.Description("Whether and how garbage collection will be performed. Either this field or OrphanDependents may be set, but not both. The default policy is decided by the existing finalizer set in the metadata.finalizers and the resource-specific default policy.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeletecollectionnamespacedcustomobjectHandler(cfg),
	}
}
