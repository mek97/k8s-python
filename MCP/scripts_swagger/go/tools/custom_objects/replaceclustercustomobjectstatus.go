...
func ReplaceclustercustomobjectstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplaceclustercustomobjectstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_group_version_plural_name_status",
		mcp.WithDescription("replace status of the cluster scoped specified custom object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReplaceclustercustomobjectstatusHandler(cfg),
	}
}
