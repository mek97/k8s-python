...
func ReplaceclustercustomobjectscaleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplaceclustercustomobjectscaleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_group_version_plural_name_scale",
		mcp.WithDescription("replace scale of the specified cluster scoped custom object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReplaceclustercustomobjectscaleHandler(cfg),
	}
}
