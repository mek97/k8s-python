...
func ReplacenamespacedcustomobjectstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplacenamespacedcustomobjectstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_group_version_namespaces_namespace_plural_name",
		mcp.WithDescription("replace status of the specified namespace scoped custom object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReplacenamespacedcustomobjectstatusHandler(cfg),
	}
}
