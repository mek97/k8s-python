...
func ReplaceclustercustomobjectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplaceclustercustomobjectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_group_version_plural_name",
		mcp.WithDescription("replace the specified cluster scoped custom object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReplaceclustercustomobjectHandler(cfg),
	}
}
