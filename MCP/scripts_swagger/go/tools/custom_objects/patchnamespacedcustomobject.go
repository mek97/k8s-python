...
func PatchnamespacedcustomobjectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePatchnamespacedcustomobjectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_apis_group_version_namespaces_namespace_plural_name",
		mcp.WithDescription("patch the specified namespace scoped custom object"),
		mcp.WithObject("body", mcp.Required(), mcp.Description("The JSON schema of the Resource to patch.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PatchnamespacedcustomobjectHandler(cfg),
	}
}
