...
func ReplacenamespacedcustomobjectscaleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReplacenamespacedcustomobjectscaleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("put_apis_group_version_namespaces_namespace_plural_name",
		mcp.WithDescription("replace scale of the specified namespace scoped custom object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReplacenamespacedcustomobjectscaleHandler(cfg),
	}
}
