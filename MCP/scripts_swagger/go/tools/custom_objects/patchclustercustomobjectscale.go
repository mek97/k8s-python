...
func PatchclustercustomobjectscaleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/%s/%s/%s/%s/scale%s", cfg.BaseURL, queryString)
...
func CreatePatchclustercustomobjectscaleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_apis_group_version_plural_name_scale",
		mcp.WithDescription("partially update scale of the specified cluster scoped custom object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PatchclustercustomobjectscaleHandler(cfg),
	}
}
