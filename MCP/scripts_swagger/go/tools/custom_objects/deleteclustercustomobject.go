...
func DeleteclustercustomobjectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/%s/%s/%s/%s%s", cfg.BaseURL, queryString)
...
func CreateDeleteclustercustomobjectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("delete_apis_group_version_plural_name",
		mcp.WithDescription("Deletes the specified cluster scoped custom object"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    DeleteclustercustomobjectHandler(cfg),
	}
}
