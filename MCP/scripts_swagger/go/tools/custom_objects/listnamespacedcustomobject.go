...
func ListnamespacedcustomobjectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListnamespacedcustomobjectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_group_version_namespaces_namespace_plural",
		mcp.WithDescription("list or watch namespace scoped custom objects"),
		...
		mcp.WithString("watch", mcp.Description("Watch for changes to the described resources and return them as a stream of add, update, and remove notifications.")),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListnamespacedcustomobjectHandler(cfg),
	}
}
