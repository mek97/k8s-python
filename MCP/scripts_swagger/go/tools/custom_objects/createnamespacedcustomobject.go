...
func CreatenamespacedcustomobjectHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateCreatenamespacedcustomobjectTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("post_apis_group_version_namespaces_namespace_plural",
		mcp.WithDescription("Creates a namespace scoped Custom object"),
		mcp.WithObject("body", mcp.Required(), mcp.Description("The JSON schema of the Resource to create.")),
	...
	return models.Tool{
		Definition: tool,
		Handler:    CreatenamespacedcustomobjectHandler(cfg),
	}
}
