...
func GetapigroupHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		url := fmt.Sprintf("%s/apis/storagemigration.k8s.io/", cfg.BaseURL)
...
func CreateGetapigroupTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_storagemigration_k8s_io",
		mcp.WithDescription("get information of a group"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetapigroupHandler(cfg),
	}
}
