...
func PatchnamespacedpodresizeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePatchnamespacedpodresizeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_api_v1_namespaces_namespace_pods_name_resize",
		mcp.WithDescription("partially update resize of the specified Pod"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PatchnamespacedpodresizeHandler(cfg),
	}
}
