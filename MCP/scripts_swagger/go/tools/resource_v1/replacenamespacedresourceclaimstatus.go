...
func ReplacenamespacedresourceclaimstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/resource.k8s.io/v1/namespaces/%s/resourceclaims/%s/status%s", cfg.BaseURL, queryString)
...
func CreateReplacenamespacedresourceclaimstatusTool(cfg *config.APIConfig) models.Tool {
	...
	return models.Tool{
		Definition: tool,
		Handler:    ReplacenamespacedresourceclaimstatusHandler(cfg),
	}
}
