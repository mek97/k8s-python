...
func PatchresourcesliceHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		url := fmt.Sprintf("%s/apis/resource.k8s.io/v1/resourceslices/%s%s", cfg.BaseURL, queryString)
...
func CreatePatchresourcesliceTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_apis_resource_k8s_io_v1_resourceslices_name",
		mcp.WithDescription("partially update the specified ResourceSlice"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PatchresourcesliceHandler(cfg),
	}
}
