...
func PatchstorageversionmigrationstatusHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreatePatchstorageversionmigrationstatusTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("patch_apis_storagemigration_k8s_io_v1alpha1",
		mcp.WithDescription("partially update status of the specified StorageVersionMigration"),
	...
	return models.Tool{
		Definition: tool,
		Handler:    PatchstorageversionmigrationstatusHandler(cfg),
	}
}
