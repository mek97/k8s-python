...
func ReadnamespacedstatefulsetHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReadnamespacedstatefulsetTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_apps_v1_namespaces_namespace_statefulsets_name_2",
		mcp.WithDescription("read the specified StatefulSet"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ReadnamespacedstatefulsetHandler(cfg),
	}
}
