...
func ReadcsinodeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateReadcsinodeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_storage_k8s_io_v1_csinodes_name",
		mcp.WithDescription("read the specified CSINode"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ReadcsinodeHandler(cfg),
	}
}
