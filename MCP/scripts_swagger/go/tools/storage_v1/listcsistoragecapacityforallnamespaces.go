...
func ListcsistoragecapacityforallnamespacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateListcsistoragecapacityforallnamespacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_storage_k8s_io_v1_csistoragecapacities",
		mcp.WithDescription("list or watch objects of kind CSIStorageCapacity"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListcsistoragecapacityforallnamespacesHandler(cfg),
	}
}
