...
func ReadclustertrustbundleHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		url := fmt.Sprintf("%s/apis/certificates.k8s.io/v1beta1/clustertrustbundles/%s", cfg.BaseURL)
...
func CreateReadclustertrustbundleTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_certificates_k8s_io_v1beta1_clustertrustbundles",
		mcp.WithDescription("read the specified ClusterTrustBundle"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ReadclustertrustbundleHandler(cfg),
	}
}
