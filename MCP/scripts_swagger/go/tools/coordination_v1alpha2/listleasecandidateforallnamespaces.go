...
func ListleasecandidateforallnamespacesHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		url := fmt.Sprintf("%s/apis/coordination.k8s.io/v1alpha2/leasecandidates", cfg.BaseURL)
...
func CreateListleasecandidateforallnamespacesTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_apis_coordination_k8s_io_v1alpha2_leasecandidates",
		mcp.WithDescription("list or watch objects of kind LeaseCandidate"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    ListleasecandidateforallnamespacesHandler(cfg),
	}
}
