...
func GetcodeHandler(cfg *config.APIConfig) func(ctx context.Context, request mcp.CallToolRequest) (*mcp.CallToolResult, error) {
...
func CreateGetcodeTool(cfg *config.APIConfig) models.Tool {
	tool := mcp.NewTool("get_version",
		mcp.WithDescription("get the version information for this server"),
	)

	return models.Tool{
		Definition: tool,
		Handler:    GetcodeHandler(cfg),
	}
}
